...
VulkanContext::VulkanContext(const Window &window, VulkanInstance &&instance, VulkanDevice &&device, VkSurfaceKHR surface,
...
VulkanContext::VulkanContext(VulkanContext &&o) noexcept
        : window(o.window), device(std::move(o.device)), instance(std::move(o.instance)), surface(o.surface),
...
VulkanContext VulkanContext::Create(const Window &window) {
    ...
    return VulkanContext{window, std::move(instance), std::move(device), surface, (std::move(commandPool))};
}
