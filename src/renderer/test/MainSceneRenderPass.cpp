...
MainSceneRenderPass::MainSceneRenderPass(VulkanDevice *device, VulkanMemory *vulkanMemory, VulkanSwapChain *swapChain) :
    ...
    VkDescriptorSetLayoutBinding textureLayBind = VkDescriptorSetLayoutBinding{ // Texture descriptor
            .binding=0,
    ...
    VkDescriptorSetLayoutBinding materialLayBind = VkDescriptorSetLayoutBinding{ // Material parameter descriptor
            .binding=1,
    ...
    m_descriptorSetLayoutMaterials = VulkanDescriptor::createDescriptorSetLayout(
            *m_device, {textureLayBind, materialLayBind});

    VkDescriptorSetLayoutBinding lightsLayBind = VkDescriptorSetLayoutBinding{ // Lights descriptor
            .binding=0,
            .descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
            .descriptorCount=1,
            .stageFlags=VK_SHADER_STAGE_FRAGMENT_BIT,
    ...
    m_descriptorSetLayoutLights = VulkanDescriptor::createDescriptorSetLayout(
            *m_device, {lightsLayBind}
    );

    // Pipeline layout
    m_graphicsPipelineLayout = PipelineLayout{
            .layouts={
                    m_descriptorSetLayoutCameraBuf, // Camera parameters + worldLight
                    m_descriptorSetLayoutLights, // Lights[3]
                    m_descriptorSetLayoutMaterials // Material parameters + texture
            },
            .pushConstants{
    ...
    m_graphicsPipeline = VulkanPipeline::create(*m_device,
                                                Vertex::getBindingDescription(),
                                                attributeDescription.data(),
                                                static_cast<uint32_t>(attributeDescription.size()),
                                                m_swapChain->getExtent(), m_graphicsPipelineLayout, m_renderPass,
                                                "base"
    );
    ...
    m_descriptorPoolMaterials = VulkanDescriptor::createPool(*m_device,
                                                             {
                                                                     VkDescriptorPoolSize{ // Texture
                                                                             .type = m_descriptorSetLayoutMaterials.bindings[0].descriptorType,
                                                                             .descriptorCount = 1024
                                                                     },
                                                                     VkDescriptorPoolSize{ // Material parameters buffer
                                                                             .type = m_descriptorSetLayoutMaterials.bindings[1].descriptorType,
                                                                             .descriptorCount = 1024
                                                                     }
                                                             });
...
void MainSceneRenderPass::createRenderPass() {
    ...
    depthAttachment.format = VulkanImage::getDepthFormat(*m_device);
...
#ifdef M_DEBUG
    std::cout << "MainSceneRenderPass: created render pass (" << m_renderPass << ")" << std::endl;
#endif
}
...
void MainSceneRenderPass::createBufferedDescriptorSetLayout() {
    ...
    m_descriptorSetLayoutCameraBuf = VulkanDescriptor::createDescriptorSetLayout(*m_device,
                                                                                 {
...
void MainSceneRenderPass::createBufferedDescriptorPool() {
    m_descriptorPoolCamera = VulkanDescriptor::createPool(*m_device,
                                                          {
                                                                  VkDescriptorPoolSize{  // Camera + worldLight
                                                                          .type=m_descriptorSetLayoutCameraBuf.bindings[0].descriptorType,
                                                                          .descriptorCount = m_swapChain->size()
                                                                  }
                                                          });
}
...
void MainSceneRenderPass::createUniformBuffers() {
    ...
    for (size_t i = 0; i < m_swapChain->size(); i++) {
        m_uniformBuffers[i] = m_vulkanMemory->createUniformBuffer(
                sizeof(UniformBufferObject),
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                1, false);
    }

    m_uboContent = UniformBufferContent<UniformBufferObject>(1);
...
void MainSceneRenderPass::createBufferedDescriptorSets() {
    ...
    m_descriptorSetsCamera.resize(m_swapChain->size());
    for (size_t i = 0; i < m_descriptorSetsCamera.size(); i++) {
        m_descriptorSetsCamera[i] = VulkanDescriptor::allocateDescriptorSet(
                *m_device, m_descriptorSetLayoutCameraBuf,
                m_descriptorPoolCamera);
    }

    // Configure the descriptors of the sets
    for (size_t i = 0; i < m_swapChain->size(); i++) {
        VulkanDescriptor::writeDescriptorSet(*m_device, m_descriptorSetsCamera[i],
                                             { // BufferInfos
...
void MainSceneRenderPass::createLightStructures() {
    ...
    m_descriptorPoolLights = VulkanDescriptor::createPool(*m_device,
                                                          {
                                                                  VkDescriptorPoolSize{
                                                                          .type = m_descriptorSetLayoutLights.bindings[0].descriptorType,
                                                                          .descriptorCount = m_swapChain->size()
                                                                  }
                                                          });

    // Create Light descriptor sets
    m_lightUniformBuffers[0] = m_vulkanMemory->createUniformBuffer(
            sizeof(UniformLightsObject),
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
    );
    m_lightsUBContent[0] = UniformBufferContent<UniformLightsObject>(1);
    // Create buffer for this descriptor set
    m_lightUniformBuffers[1] = m_vulkanMemory->createUniformBuffer(
            sizeof(UniformLightsObject),
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
    );
    m_lightsUBContent[1] = UniformBufferContent<UniformLightsObject>(1);

    UniformLightsObject *lights1 = m_lightsUBContent[0].at();
    ...
    m_vulkanMemory->copyDataToBuffer(m_lightUniformBuffers[0].buffer, m_lightUniformBuffers[0].memory, lights1,
                                     sizeof(UniformLightsObject));
    // Fill Lights buffer
    UniformLightsObject *lights2 = m_lightsUBContent[1].at();
    std::memcpy(lights2, lights1, sizeof(UniformLightsObject));
    m_vulkanMemory->copyDataToBuffer(m_lightUniformBuffers[1].buffer, m_lightUniformBuffers[1].memory, lights2,
                                     sizeof(UniformLightsObject));

    // Write the descriptor sets
    m_descriptorSetsLights.resize(m_swapChain->size());
    for (size_t i = 0; i < m_swapChain->size(); i++) {
        m_descriptorSetsLights[i] = VulkanDescriptor::allocateDescriptorSet(*m_device,
                                                                            m_descriptorSetLayoutLights,
                                                                            m_descriptorPoolLights);

        VulkanDescriptor::writeDescriptorSet(*m_device, m_descriptorSetsLights[i],
                                             {
                                                     DescriptorBufferInfo{
                                                             .descriptorInfo = VkDescriptorBufferInfo{
                                                                     .buffer = m_lightUniformBuffers[0].buffer,
                                                                     .offset = 0,
                                                                     .range = VK_WHOLE_SIZE
                                                             },
                                                             .binding = 0,
...
void MainSceneRenderPass::updateUniformBuffer(uint32_t currentImage,
    ...
    UniformBufferObject *ubo = m_uboContent.at(0);
    ...
    ubo->proj = glm::perspective(glm::radians(camera.angle),
                                 ...
                                 (float) m_swapChain->getExtent().height, camera.near, camera.far);
    ubo->proj[1][1] *= -1;
    ubo->worldLightPosition = worldLight.lightPos;
    ubo->worldLightColor = worldLight.lightColor;

    // Copy that data to the uniform buffer
    m_vulkanMemory->copyDataToBuffer(m_uniformBuffers[currentImage].buffer,
                                     m_uniformBuffers[currentImage].memory,
                                     m_uboContent.data(), m_uboContent.size());
}

// Rendering stuff
/* Begin the render pass and setup all general descriptors . */
void MainSceneRenderPass::cmdBegin(VkCommandBuffer &cmdBuf, uint32_t currentImage, VkFramebuffer framebuffer) {
    ...
    vkCmdBindDescriptorSets(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            ...
                            0, 1, &m_descriptorSetsCamera[currentImage], 0, nullptr);
    // Bind the lights descriptor set
    vkCmdBindDescriptorSets(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            m_graphicsPipeline.pipelineLayout,
                            1, 1, &m_descriptorSetsLights[currentImage], 0, nullptr);
}
...
void MainSceneRenderPass::cmdRender(VkCommandBuffer &cmdBuf, RenderObject &robj) {
    ...
    if (robj.material < m_descriptorSetsMaterials.size()) {
        // Bind the material descriptor set to the pipeline // TOBE: not per renderobject
        vkCmdBindDescriptorSets(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                m_graphicsPipeline.pipelineLayout,
                                2, 1, &m_descriptorSetsMaterials[robj.material], 0, nullptr);
    } else {
    ...
    vkCmdPushConstants(cmdBuf, m_graphicsPipeline.pipelineLayout,
...
void MainSceneRenderPass::destroySwapChainDependent() {
    ...
    for (size_t i = 0; i < m_uniformBuffers.size(); i++) {
        m_vulkanMemory->destroy(m_uniformBuffers[i]);
    }

    // The descriptor pool and sets depend also on the number of images in the swapchain
    vkDestroyDescriptorPool(m_device->getDevice(), m_descriptorPoolCamera,
...
void MainSceneRenderPass::destroy() {
    ...
    vkDestroyDescriptorPool(m_device->getDevice(), m_descriptorPoolMaterials, nullptr);
    vkDestroyDescriptorPool(m_device->getDevice(), m_descriptorPoolLights, nullptr);

    vkDestroyDescriptorSetLayout(m_device->getDevice(), m_descriptorSetLayoutCameraBuf.vDescriptorSetLayout, nullptr);
    vkDestroyDescriptorSetLayout(m_device->getDevice(), m_descriptorSetLayoutMaterials.vDescriptorSetLayout, nullptr);
    vkDestroyDescriptorSetLayout(m_device->getDevice(), m_descriptorSetLayoutLights.vDescriptorSetLayout, nullptr);

    for (size_t i = 0; i < m_materialUniformBuffers.size(); i++) {
        m_vulkanMemory->destroy(m_materialUniformBuffers[i]);
    }
    for (size_t i = 0; i < m_lightUniformBuffers.size(); i++) {
        m_vulkanMemory->destroy(m_lightUniformBuffers[i]);
    }

    for (auto &t : m_textures) {
        VulkanTexture::destroy(*m_device, t.second);
    }

    vkDestroyRenderPass(m_device->getDevice(), m_renderPass, nullptr);

    m_uboContent.destroy();
}

////////////////////////////////////////////////////////////////////
MaterialRef MainSceneRenderPass::createMaterial(const TexturePhongMaterial material) {
    // Load texture
    auto result = m_textures.find(material.textureFile);
    VulkanTexture texture;
    if (result == m_textures.end()) {
        texture = VulkanTexture::createTexture(
                *m_device, *m_vulkanMemory, "textures/" + material.textureFile);
        m_textures[material.textureFile] = texture;
    } else {
    ...
    VulkanUniformBuffer materialBuffer = m_vulkanMemory->createUniformBuffer(
            sizeof(UniformMaterialObject),
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT
    );
    m_materialUniformBuffers.emplace_back(materialBuffer);
    m_materialUBContent.emplace_back(
            UniformBufferContent<UniformMaterialObject>(1)
    );
    UniformMaterialObject *matBufferPointer = m_materialUBContent[m_materialUBContent.size() - 1].at();
    matBufferPointer->shininess = material.shininess;
    m_vulkanMemory->copyDataToBuffer(materialBuffer.buffer, materialBuffer.memory,
                                     matBufferPointer, sizeof(UniformMaterialObject));

    // Create and write descriptor set for this material
    m_descriptorSetsMaterials.emplace_back(
            VulkanDescriptor::allocateDescriptorSet(*m_device,
                                                    m_descriptorSetLayoutMaterials, m_descriptorPoolMaterials)
    );
    MaterialRef ref = m_descriptorSetsMaterials.size() - 1;
    VulkanDescriptor::writeDescriptorSet(*m_device, m_descriptorSetsMaterials[ref],
                                         {
                                                 DescriptorBufferInfo{
                                                         .descriptorInfo = VkDescriptorBufferInfo{
