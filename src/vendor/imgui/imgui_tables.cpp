...
ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2 &outer_size,
    ...
    table->IsOuterRectMinFitX = (outer_size.x == 0.0f) && (use_child_window ==
                                                           false); // Will be set to false later if there are any Stretch column.
...
void ImGui::EndTable() {
    ...
    // CursorPosPrevLine and CursorMaxPos manually. That should be a more context layout feature, see same problem e.g. #3414)
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX)
        outer_window->DC.CursorMaxPos.x = ImMax(backup_outer_max_pos.x,
                                                table->OuterRect.Min.x + table->ColumnsGivenWidth +
                                                inner_window->ScrollbarSizes.x); // For outer scrolling
    else
        outer_window->DC.CursorMaxPos.x = ImMax(backup_outer_max_pos.x,
                                                table->WorkRect.Min.x + outer_width); // For auto-fit
