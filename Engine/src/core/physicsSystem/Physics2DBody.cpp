...
namespace ChaosEngine {
    ...
    void Physics2DBody::setPosition(const glm::vec3 &newPosition) {
    ...
    void Physics2DBody::setAngularVelocity(const glm::vec3 &velocity) {
        body->SetAngularVelocity(velocity.z);
    }


    static std::unique_ptr<b2Shape>
    getBox2DShape(const RigidBody2D::RigitBody2DShape &shape, const Transform &transform) {
        constexpr float skin = b2_polygonRadius / 2;

        glm::vec2 dimension = shape.dimension.value_or(glm::vec2(transform.scale));

        switch (shape.type) {
            case RigidBody2D::Box: {
                auto ret = std::make_unique<b2PolygonShape>();
                ret->SetAsBox(dimension.x - skin, dimension.y - skin);
                return ret;
            }
            case RigidBody2D::Cricle: {
                auto ret = std::make_unique<b2CircleShape>();
                ret->m_p.SetZero();
                ret->m_radius = dimension.x - skin;
                return ret;
            }
            default:
    ...
    StaticRigidBodyComponent RigidBody2D::CreateStaticRigidBody(Entity &entity, const RigitBody2DShape &shapeDef) {
        ...
        auto shape = getBox2DShape(shapeDef, transform);
